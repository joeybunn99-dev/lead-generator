 ...
 * @param {string} domain
 * @returns {Promise<string[]>} Valid generic emails found
 */
 ...
 * @param {string} domain
 * @returns {Promise<string|null>} First valid email found, or null
 */
async function guessContactEmail(firstName, lastName, domain) {
  ...
  for (const pattern of NAME_PATTERNS) {
    ...
    if (catchAll) {
      // Can't verify — return the most common pattern
  ...
  for (const co of companies) {
    ...
    try {
      ...
      for (const email of emails) {
        // Insert as a contact
        await run(
          `INSERT INTO contacts (company_id, name, title, email, email_status, source)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [co.id, co.name, 'General Contact', email, 'valid', 'email-discovery']
        );
        found++;
      }
